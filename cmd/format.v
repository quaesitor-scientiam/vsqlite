...
pub fn format_opts(rows []Row, opts FormatOptions) string {
...
fn compute_widths(rows []Row, cols []string, opts FormatOptions) []int {
	ncols := cols.len
	mut auto_w := []int{len: ncols, init: cols[index].len}
	for row in rows {
...
fn format_table_ex(rows []Row, opts FormatOptions) string {
	if rows.len == 0 {
		return ''
	}
	ncols := rows[0].vals.len
	cols := resolve_cols(rows[0].cols, ncols)
	widths := compute_widths(rows, cols, opts)
...
fn format_csv_ex(rows []Row, opts FormatOptions) string {
	if rows.len == 0 {
		return ''
	}
	ncols := rows[0].vals.len
	...
	mut lines := []string{}
	if opts.headers {
		lines << cols.map(csv_escape_sep(it, sep)).join(sep)
	}
	for row in rows {
...
fn format_line_ex(rows []Row, opts FormatOptions) string {
	if rows.len == 0 {
		return ''
	}
	ncols := rows[0].vals.len
	...
	for row in rows {
		...
		for j, val in row.vals {
			...
			lines << '${str_pad_left(cols[j], max_len)}: ${null_disp(val, opts.nullvalue)}'
		}
		blocks << lines.join('\n')
	}
	return blocks.join('\n\n')
...
fn format_box_ex(rows []Row, opts FormatOptions) string {
	if rows.len == 0 {
		return ''
	}
	ncols := rows[0].vals.len
...
fn format_markdown_ex(rows []Row, opts FormatOptions) string {
	if rows.len == 0 {
		return ''
	}
	ncols := rows[0].vals.len
	cols := resolve_cols(rows[0].cols, ncols)
	raw_w := compute_widths(rows, cols, opts)
	mut widths := []int{len: ncols}
	for i, w in raw_w {
...
fn format_json_ex(rows []Row, opts FormatOptions) string {
	if rows.len == 0 {
		return '[]'
	}
	ncols := rows[0].vals.len
...
fn format_html_ex(rows []Row, opts FormatOptions) string {
	if rows.len == 0 {
		return ''
	}
	ncols := rows[0].vals.len
...
fn format_insert_ex(rows []Row, opts FormatOptions) string {
	if rows.len == 0 {
		return ''
	}
	ncols := rows[0].vals.len
...
fn format_quote_ex(rows []Row, opts FormatOptions) string {
	if rows.len == 0 {
		return ''
	}
	ncols := rows[0].vals.len
